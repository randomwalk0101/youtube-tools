...
if ($item.Kind -eq "manual") {
    yt-dlp.exe $clean --no-playlist --skip-download --write-subs --sub-langs $item.Lang --sub-format "vtt/best" -o "$tmpdir\sub.%(ext)s"
} else {
    yt-dlp.exe $clean --no-playlist --skip-download --write-auto-subs --sub-langs $item.Lang --sub-format "vtt/best" -o "$tmpdir\sub.%(ext)s"
}
